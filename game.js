...
function makethespinhappen() {
  ...
  if (curtime - prevtime > interval) {
    ...
    infinitus.attack(Math.floor(2 * Math.random()));
    infinitus.show();
  ...
  constructor(cx, cy, rad, rot) {
    ...
    this.attacks.push(function(num) {
      ...
      for (let i = 0; i < num; i++) {
        ...
        if (Math.random() > .7) {
          enemy_bullets.push(new Bullet(nx, this.y + this.r, 5, enemy_bullet_vel * 3, traj));
        }
...
leaderboard menu
more attacks - infinitus dark side
**********sound**********
